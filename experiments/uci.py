...
from nde.transforms import CPABCouplingTransform

parser = argparse.ArgumentParser()

enable_cuda = True
device_type = 'gpu' if torch.cuda.is_available() and enable_cuda else 'cpu'
...
def create_base_transform(i):
    ...
    elif args.base_transform_type == 'cpab-coupling':
        return CPABCouplingTransform(
            mask= utils.create_alternating_binary_mask(features, even=(i % 2 == 0)),
            transform_net_create_fn=lambda in_features, out_features: nn_.ResidualNet(
                in_features=in_features,
                out_features=out_features,
            ...
            T = difw.Cpab(args.num_bins, backend='pytorch', device= device_type, zero_boundary=True, basis="qr"),
            num_bins=args.num_bins,
            tails='linear',
            tail_bound=args.tail_bound,
            apply_unconditional_transform=args.apply_unconditional_transform
