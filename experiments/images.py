...
runs_dir = os.path.join(utils.get_data_root(), 'image_modeling')
...
def config():
    ...
    # Coupling transform
    coupling_layer_type = 'cpab_spline'
    spline_params = {
        'num_bins': 4,
        'tail_bound': 1.,
        'min_bin_width': 1e-3,
    ...
    multi_gpu = False
    run_descr = ''
    flow_checkpoint = 'cpab-cifar-10-5'
...
def create_transform_step(num_channels,
    ...
    elif coupling_layer_type == 'cpab_spline':
        coupling_layer = transforms.CPABCouplingTransform(
            mask=mask,
            transform_net_create_fn=create_convnet,
            tails='linear',
            tail_bound=spline_params['tail_bound'],
            num_bins=spline_params['num_bins'],
            apply_unconditional_transform=spline_params['apply_unconditional_transform'],
            T= difw.Cpab(spline_params['num_bins'], backend='pytorch', device= 'gpu', zero_boundary=True, basis="qr")
        )
...
def create_flow(c, h, w,
    ...
    if flow_checkpoint is not None:
        ...
        flow.load_state_dict(torch.load(runs_dir + '/cifar-10-fast-8-bit-1/flow_best.pt'))
    return flow
