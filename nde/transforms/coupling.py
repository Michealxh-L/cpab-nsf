...
from nde.transforms.splines import unconstrained_cpab_spline, cpab_spline
...
class CPABCouplingTransform(PiecewiseCouplingTransform):
    ...
    def __init__(
        ...
        if apply_unconditional_transform:
            unconditional_transform = lambda features: transforms.ContinuousPiecewiseAffineCDF(
                shape=[features] + (img_shape if img_shape else []),
                num_bins=num_bins,
                T = T,
                tails=tails,
