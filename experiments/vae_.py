...
enable_cuda = True
device_type = 'cuda' if torch.cuda.is_available() and enable_cuda else 'cpu'
...
# features size
parser.add_argument('--latent_features', type=int, default=64,
...
# prior choices
parser.add_argument('--prior_type', type=str, default='cpab-coupling',
                    choices=['standard-normal', 'affine-coupling',
                             'rq-coupling', 'affine-autoregressive',
                             'rq-autoregressive','cpab-coupling'],
                    help='Which prior to use.')

# approximate posterior
parser.add_argument('--approximate_posterior_type', type=str, default='cpab-coupling',
...
def run(seed):
    ...
    def create_base_transform(i, context_features=None):
        ...
        elif args.prior_type == 'cpab-coupling':
            return transforms.CPABCouplingTransform(
                mask= utils.create_alternating_binary_mask(args.latent_features, even=(i % 2 == 0)),
                ...
                T = difw.Cpab(args.num_bins, backend='pytorch', device= 'gpu', zero_boundary=True, basis="qr"),
                num_bins=args.num_bins,
                tails='linear',
                tail_bound=args.tail_bound,
                apply_unconditional_transform=args.apply_unconditional_transform
    ...
    # log_dir = os.path.join('logs', args.dataset_name, timestamp)
    log_dir = 'logs/' + args.dataset_name + args.prior_type
    while True:
        try:
